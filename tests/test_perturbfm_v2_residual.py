...
def _make_residual_dataset() -> PerturbDataset:
    delta = np.array(
        [
            ...
            [0.0, 1.0, 0.0],  # B
            [0.0, 0.0, 1.0],  # C
            [1.5, 1.5, 0.5],  # A+B (additive + residual)
            [1.5, 0.5, 1.5],  # A+C (additive + residual)
        ],
        dtype=np.float32,
    )
    obs = {
        "pert_id": ["A", "B", "C", "A+B", "A+C"],
        "context_id": ["C0"] * 5,
        "batch_id": ["B0"] * 5,
        "is_control": [False] * 5,
        "pert_genes": [["A"], ["B"], ["C"], ["A", "B"], ["A", "C"]],
    }
    var = ["A", "B", "C"]
...
def test_v2_residual_beats_additive_on_combo():
